...
Architecture:
  • Single ``YOUTUBE_API_KEY`` env var. Single ``YOUTUBE_CHANNEL_ID``.
...
async def _yt_get(client: httpx.AsyncClient, path: str, params: dict) -> dict:
    ...
    r = await client.get(f"{YOUTUBE_API_BASE}/{path}", params=params, timeout=20.0)
    if r.status_code >= 400:
        # Surface a useful error to the admin instead of swallowing it.
        try:
            body = r.json()
...
async def _fetch_channel_playlists(client: httpx.AsyncClient) -> list[dict]:
    ...
    while True:
        ...
        data = await _yt_get(client, "playlists", params)
...
async def _fetch_playlist_videos(client: httpx.AsyncClient, playlist_id: str) -> list[dict]:
    ...
    while True:
        ...
        if page_token:
            params["pageToken"] = page_token
        data = await _yt_get(client, "playlistItems", params)
...
async def _sync_all_playlists(db, triggered_by: str) -> dict:
    ...
    Returns the sync-log summary. Errors are caught and logged but the
    existing cache is left intact — partial failures still report.
    """
    ...
    try:
        async with httpx.AsyncClient() as client:
            playlists = await _fetch_channel_playlists(client)
            ...
            for pl in playlists:
                ...
                if not yt_id:
                    continue
                try:
                    videos = await _fetch_playlist_videos(client, yt_id)
                ...
                if existing:
                    ...
                    # Default: disabled + uncategorised. Admin must
                    # explicitly opt-in before it appears in the portal.
                    doc["category"] = None
                    doc["enabled"] = False
                    doc["sort_order"] = 0
...
def attach(api: APIRouter, db, require_role):
    ...
    async def admin_refresh_one(
        ...
        if not existing:
            raise HTTPException(404, "Playlist not found")
        async with httpx.AsyncClient() as client:
            videos = await _fetch_playlist_videos(client, existing["youtube_id"])
        await db.youtube_playlists.update_one(
            {"id": playlist_id},
...
async def start_scheduler(db):
    """Run a daily sync at ~03:00 UTC. Uses the same logic as the manual
    admin button, just triggered by the scheduler. Survives YouTube
    being down (errors stored in sync log, cache untouched).
    """
    async def _loop():
        while True:
            try:
                ...
                await asyncio.sleep(3600)  # back off an hour on unexpected error

...
# Rule + filter helpers live in dedicated leaf modules — both router
# files depend on these leaves rather than each other (removes the
# cqc_routes ↔ scotland_routes circular import the code review flagged).
...
from cqc_definition import (
    CqcDefinition,
    DEFAULT_DEFINITION_ID as CQC_DEFAULT_DEFINITION_ID,
    definition_to_mongo_filter as cqc_definition_to_mongo_filter,
)
...
def build_scotland_router(db, require_role):  # noqa: D401
    ...
    async def put_definition(body: ScotlandDefinition, user: dict = Depends(require_role("admin"))):
        ...
        async for f in cur:
            ...
            # straddling the border keep their full number. ``CqcDefinition``
            # + filter helper live in a dedicated leaf module — no more
            # cross-router lazy import.
            cqc_doc = await db.cqc_definition.find_one({"_id": CQC_DEFAULT_DEFINITION_ID}, {"_id": 0})
            ...
            if eng_sectors:
                eng_cnt = await db.cqc_locations_live.count_documents({
                    **cqc_definition_to_mongo_filter(cqc_def), "postcode_sector": {"$in": eng_sectors},
                })
                if eng_cnt == 0:

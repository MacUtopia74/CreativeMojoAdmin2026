...
import { ArrowLeft, MapPin, AlertCircle, User, FileText, Map, MessageSquare, Pencil, Check, X as XIcon, Clock, ShieldCheck, ShieldAlert, Globe, Facebook, CreditCard, RefreshCw, AlertTriangle, Power, PowerOff, BellRing, FolderOpen, LockKeyhole, Calendar, Camera, Loader2, ClipboardList, BadgeCheck, Home, Mail, Phone } from "lucide-react";
...
// ---------------------------------------------------------------------------
// Care-home enquiries overlay — wraps the Territory Map panel with a toggle
// button, date-preset bar, and compact list. When ON, we overlay pins on
// the *existing* TerritoryMap (via extraPins) rather than instantiating a
// second map (saves Mapbox credits). Only shows enquiries whose postcode
// resolves inside this franchisee's saved sectors — server-side filter
// via /api/contacts/map?source=care_home_enquiry&franchisee_id=<fid>.
// ---------------------------------------------------------------------------
function CareHomeEnquiriesOverlay({ franchiseeId, children }) {
  ...
  useEffect(() => {
    if (!enabled || !franchiseeId) { setPins([]); return; }
    let alive = true;
    setLoading(true); setError("");
    (async () => {
      try {
        const params = { source: "care_home_enquiry", franchisee_id: franchiseeId };
        if (range.from) params.date_from = range.from;
        if (range.to) params.date_to = range.to;
        const { data } = await api.get("/contacts/map", { params });
        if (!alive) return;
        setPins(data.pins || []);
      } catch (e) {
    ...
    return () => { alive = false; };
  }, [enabled, franchiseeId, range.from, range.to]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border border-stone-200 bg-stone-50/60 rounded-xl p-3 space-y-2">
        <div className="flex flex-wrap items-center gap-2 justify-between">
          <button
            ...
            {enabled ? "Hide Care Home Enquiries" : "Show Care Home Enquiries in this area"}
          </button>
          {enabled && (
      ...
      {/* Territory map (with optional care-home pins via extraPins) */}
      {typeof children === "function"
        ? children({ extraPins: enabled ? extraPins : null, hoverId, setHoverId })
        : children}

      {/* Enquiry list */}
      {enabled && (
        <div className="border border-stone-200 rounded-xl overflow-hidden" data-testid="care-home-overlay-list">
          {error ? (

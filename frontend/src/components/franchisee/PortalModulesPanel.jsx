...
import { Map as MapIcon, Calendar, Folder, FileText, Loader2 } from "lucide-react";
...
const MODULES = [
  ...
  { key: "files",     label: "Files",          icon: Folder,   description: "Brand assets, marketing materials, ops docs." },
  { key: "invoicing", label: "Invoicing",      icon: FileText, description: "Personal invoicing tool + CSV bank reconciliation." },
];
...
export default function PortalModulesPanel({ franchisee, onChanged }) {
  ...
  const [state, setState] = useState({
    ...
    invoicing: initial.invoicing === true,   // default OFF
  });
  ...
  const toggle = async (key) => {
    ...
    try {
      ...
      toast.success(`${MODULES.find((m) => m.key === key).label} ${next[key] ? "enabled" : "disabled"}`);
      onChanged?.();
    } catch (e) {
  ...
  return (
    <div className="space-y-2" data-testid="portal-modules-panel">
